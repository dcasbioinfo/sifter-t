...
def get_fullgeneset(gene_ac):
    ...
    for pf in gene_ac:
        if i >= 250:
            temp_set = temp_set | eval(fullgeneset)
            fullgeneset = "set() "
            i = 0
        fullgeneset = fullgeneset+"| set(gene_ac[\""+pf+"\"]) "
        i = i + 1
    fullgeneset = temp_set | eval(fullgeneset)
    del(temp_set)
    return fullgeneset
...
def write_pf_noannot(options, annot_gene_set, fullfamilyset):
    ...
    for pf in fullfamilyset:
        ...
        pfam_genes[pf] = set()
        for line in handle:
            pfam_genes[pf].add(line.strip())
        handle.close()    
    # finding wich family have no annotations at all
    no_annot = set()
    for pf in pfam_genes:
        gene_set = set()
        for gene in pfam_genes[pf]:
            if gene[0:gene.find("/")] in annot_gene_set:
                gene_set.add(gene)
        if len(gene_set) == 0:
            no_annot.add(pf)
